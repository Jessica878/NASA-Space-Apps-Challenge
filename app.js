...
(() => {
  ...
  const DEBUG = true; // set true for verbose logs
  ...
  (async function main(){
    try{
      ...
      const TILE_PATHS = {
        ...
        ir: 'https://astrogeology.usgs.gov/mapcache/wmts/1.0.0/lunar_lro_diviner_thermal_global_64ppd_v02/default/default028mm/{z}/{y}/{x}.png',
        elev: 'https://astrogeology.usgs.gov/mapcache/wmts/1.0.0/lunar_lro_lola_global_ldem_118m_v02/default/default028mm/{z}/{y}/{x}.png',
        index: 'https://astrogeology.usgs.gov/mapcache/wmts/1.0.0/lunar_lro_mini_rf_global_70cm_v02/default/default028mm/{z}/{y}/{x}.png',
      };
      ...
      const layerIR = makeTileLayer(TILE_PATHS.ir, { opacity: 0.8, maxZoom: 6 });
      const layerElev = makeTileLayer(TILE_PATHS.elev, { opacity: 0.8, maxZoom: 6 });
      const layerIndex = makeTileLayer(TILE_PATHS.index, { opacity: 0.8, maxZoom: 6 });
      ...
      if(DEBUG) console.log('Sample feature:', features[0]);
      ...
      function makeMarkerForFeature(f){
        ...
        marker.on('click', (e)=>{
          // ensure normalized
          const ff = f;
          if(DEBUG) console.log('Marker clicked:', ff);
          // open popup
          const popupHtml = buildPopupHtml(ff);
          marker.bindPopup(popupHtml, { minWidth: 280, maxWidth: 320 }).openPopup();
          ...
          // also show details in right panel
          showCraterDetails(ff);
        });
        return marker;
      }
      ...
      function buildPopupHtml(f){
        ...
        const latLon = f.lat != null && f.lon != null ? `${f.lat.toFixed(4)}, ${f.lon.toFixed(4)}` : '—';
        
        // Water score color coding
        let scoreColor = '#666';
        if(score > 0.7) scoreColor = '#ff6b6b';
        else if(score > 0.3) scoreColor = '#ffd166';
        else if(score > 0) scoreColor = '#2b6cff';
        
        return `
          <div class="popup-card">
            <h4 style="margin:0 0 6px 0;">${escapeHtml(f.name || f.id || 'Feature')}</h4>
            <table class="popup-table" style="width:100%;font-size:13px;">
              <tr><td>Water Score</td><td style="text-align:right;color:${scoreColor}"><b>${score.toFixed(3)}</b></td></tr>
              <tr><td>PSR Overlap</td><td style="text-align:right;">${psr}</td></tr>
              <tr><td>Diameter</td><td style="text-align:right;">${diamDisplay}</td></tr>
              <tr><td>Spectral Mean</td><td style="text-align:right;">${spectral}</td></tr>
              <tr><td>Hydrogen Mean</td><td style="text-align:right;">${hydrogen}</td></tr>
              <tr><td>Depth Metric</td><td style="text-align:right;">${depth}</td></tr>
              <tr><td>Coordinates</td><td style="text-align:right;font-size:11px;">${latLon}</td></tr>
            </table>
            <div style="margin-top:8px;text-align:right;">
              ...
              <button class="btn small details-btn" data-id="${escapeHtml(String(f.id))}" style="margin-left:6px">Details</button>
            </div>
          </div>`;
      }
      ...
      // show detailed crater information in right panel
      function showCraterDetails(f){
        const featureDetails = document.getElementById('featureDetails');
        const craterInfo = document.getElementById('craterInfo');
        
        if(!featureDetails || !craterInfo) return;
        
        // Hide empty state, show crater info
        featureDetails.style.display = 'none';
        craterInfo.style.display = 'block';
        
        // Update all the information fields
        document.getElementById('craterName').textContent = f.name || f.id || 'Unnamed Crater';
        document.getElementById('craterId').textContent = f.id || '—';
        
        const diameter = f.diameter_m ? `${Math.round(f.diameter_m)} m (${(f.diameter_m/1000).toFixed(2)} km)` : '—';
        document.getElementById('craterDiameter').textContent = diameter;
        
        const depth = f.depth_metric ? `${f.depth_metric.toFixed(3)}` : '—';
        document.getElementById('craterDepth').textContent = depth;
        
        const waterScore = f.water_score != null ? f.water_score.toFixed(3) : '—';
        document.getElementById('waterScore').textContent = waterScore;
        
        const psr = f.psr_overlap ? 'Yes' : 'No';
        document.getElementById('psrOverlap').textContent = psr;
        
        const spectral = f.spectral_mean ? f.spectral_mean.toFixed(3) : '—';
        document.getElementById('spectralMean').textContent = spectral;
        
        const hydrogen = f.hydrogen_mean ? f.hydrogen_mean.toFixed(3) : '—';
        document.getElementById('hydrogenMean').textContent = hydrogen;
        
        const lat = f.lat ? f.lat.toFixed(6) : '—';
        const lon = f.lon ? f.lon.toFixed(6) : '—';
        document.getElementById('craterLat').textContent = lat;
        document.getElementById('craterLon').textContent = lon;
        document.getElementById('craterCoords').textContent = `${lat}, ${lon}`;
        
        // Generate analysis text
        const analysis = generateCraterAnalysis(f);
        document.getElementById('craterAnalysis').innerHTML = analysis;
      }
      
      // generate analysis text for crater
      function generateCraterAnalysis(f){
        const score = f.water_score || 0;
        const diameter = f.diameter_m || 0;
        const psr = f.psr_overlap;
        
        let analysis = '<p>';
        
        // Water likelihood assessment
        if(score > 0.7) {
          analysis += '<strong style="color: #ff6b6b;">High water likelihood</strong> - This crater shows strong indicators of water presence. ';
        } else if(score > 0.3) {
          analysis += '<strong style="color: #ffd166;">Moderate water likelihood</strong> - This crater shows some indicators of water presence. ';
        } else if(score > 0) {
          analysis += '<strong style="color: #2b6cff;">Low water likelihood</strong> - This crater shows minimal indicators of water presence. ';
        } else {
          analysis += '<strong style="color: #666;">No water data</strong> - Water likelihood assessment not available. ';
        }
        
        // Size analysis
        if(diameter > 1000000) {
          analysis += 'This is a large crater (>1km diameter) which may have better preservation of volatiles. ';
        } else if(diameter > 100000) {
          analysis += 'This is a medium-sized crater which may contain some volatile deposits. ';
        } else if(diameter > 0) {
          analysis += 'This is a small crater with limited volatile preservation potential. ';
        }
        
        // PSR analysis
        if(psr) {
          analysis += 'The crater overlaps with a Permanently Shadowed Region (PSR), which is favorable for water ice preservation. ';
        } else {
          analysis += 'The crater does not overlap with a Permanently Shadowed Region. ';
        }
        
        // Additional scientific data
        if(f.spectral_mean != null) {
          analysis += `Spectral analysis shows a mean value of ${f.spectral_mean.toFixed(3)}. `;
        }
        
        if(f.hydrogen_mean != null) {
          analysis += `Hydrogen content analysis shows a mean value of ${f.hydrogen_mean.toFixed(3)}. `;
        }
      ...
      function renderFeatures(list){
        ...
        for(const f of list){
          ...
          // attach popupopen behaviour: wire accept, comment, and details buttons safely
      ...
      let currentLayer = layerVis;
      
      radios.forEach(r => r.addEventListener('change', e=>{
        const v = e.target.value;
        map.removeLayer(layerVis); map.removeLayer(layerIR); map.removeLayer(layerElev); map.removeLayer(layerIndex);
        if(v==='vis') { map.addLayer(layerVis); currentLayer = layerVis; }
        else if(v==='ir') { map.addLayer(layerIR); currentLayer = layerIR; }
        else if(v==='elev') { map.addLayer(layerElev); currentLayer = layerElev; }
        else if(v==='index') { map.addLayer(layerIndex); currentLayer = layerIndex; }
      }));
      ...
      if(opacitySlider && opacityValue) {
        opacitySlider.addEventListener('input', e => {
          ...
          if(currentLayer) {
            ...
            if(DEBUG) console.log('Set opacity to:', opacity);
          }
        });
      }
      ...
      if(blendMode) {
        blendMode.addEventListener('change', e => {
          ...
          if(currentLayer) {
            // Apply CSS blend mode to the layer
            const tiles = currentLayer.getContainer();
            if(tiles) {
              tiles.style.mixBlendMode = mode;
              if(DEBUG) console.log('Set blend mode to:', mode);
            }
          }
        });
      }

      // Add visual feedback for layer controls
      ...
      setTimeout(() => {
        if(currentLayer) {
          toast('Layer controls active - try adjusting opacity and blend modes!');
        }
      }, 2000);
